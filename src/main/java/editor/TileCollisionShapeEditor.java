...
final class TileCollisionShapeEditor {
    private enum Mode {
        AddPolygon(() -> {
    ...
    static void renderLayerCollisionShape(TileSet tileset, Tile tile, List<Tile> selectedTiles) {
        ...
        for (Vector2f node : nodes) {
            ...
            if (!tmp.equals(node)) {
                node.set(Math.clamp(tmp.x, 0.0f, 1.0f), Math.clamp(tmp.y, 0.0f, 1.0f));
                edited = true;
            }
        }
        if (!edited) return;
    ...
    private static void drawPendingNodes(Vector2f size) {
        ...
        for (int i = 0; i < screenNodes.size() - 1; i++) drawList.addLine(screenNodes.get(i), screenNodes.get(i + 1), PendingLineColor, 2.0f);
        ...
        for (ImVec2 node : screenNodes) drawList.addCircleFilled(node, NodeRadius, PendingNodeColor);
    }
    ...
    private static void handleAddMode(Vector2f size, Tile tile) {
        ...
        if (outsidePreview(size, mousePos, screenCursorPos)) {
            hoveringEdge = -1;
            return;
        }
    ...
    private static void handleEditMode(Vector2f size, Tile tile) {
        ...
        boolean isMouseClick = ImGui.isMouseClicked(ImGuiMouseButton.Left);
        if (outsidePreview(size, mousePos, screenCursorPos)) return;
        Vector2f[] nodes = tile.collisionPolygonNodes;
        if (isMouseClick) {
            for (int i = 0; i < nodes.length; i++) {
                if (outsideNodeRadius(size, screenCursorPos, mousePos, nodes[i])) continue;
                draggingNodeIndex = i;
                break;
            }
        }
    ...
    private static void handleRemoveNode(Vector2f size, Tile tile) {
        ...
        if (outsidePreview(size, mousePos, screenCursorPos) || (!leftClick && !rightClick)) return;
        Vector2f[] nodes = tile.collisionPolygonNodes;
        for (int i = 0; i < nodes.length; i++) {
            if (outsideNodeRadius(size, screenCursorPos, mousePos, nodes[i])) continue;
            if (rightClick) {
    ...
    private static boolean closingPolygon(Vector2f size, ImVec2 screenCursorPos, ImVec2 mousePos) {
        ...
        if (outsideNodeRadius(size, screenCursorPos, mousePos, firstNode)) return false;
        addPendingNodes();
        draggingNewNode = false;
        return true;
    }
    ...
    private static void insertNodeToEdge(Tile tile, Vector2f size, ImVec2 screenCursorPos, ImVec2 mousePos, int edgeIndex) {
        ...
        if (currentNodes.length - (edgeIndex + 1) >= 0) System.arraycopy(currentNodes, edgeIndex + 1, newNodes, edgeIndex + 1 + 1, currentNodes.length - (edgeIndex + 1));
        tile.collisionPolygonNodes = newNodes;
        edited = true;
        hoveringEdge = -1;
    }
    ...
    private static Vector2f getNormal(ImVec2 mousePos, ImVec2 screenCursorPos, Vector2f size) {
        ...
        normalX = Math.clamp(normalX, 0.0f, 1.0f);
        normalY = Math.clamp(normalY, 0.0f, 1.0f);
        return new Vector2f(normalX, normalY);
    }

    private static boolean outsidePreview(Vector2f size, ImVec2 mousePos, ImVec2 screenCursorPos) {
    ...
    private static boolean outsideNodeRadius(Vector2f size, ImVec2 screenCursorPos, ImVec2 mousePos, Vector2f nodes) {
    ...
    private static List<int[]> triangulatePolygon(List<ImVec2> vertices) {
        ...
        for (int i = 0; i < vertices.size(); i++) indices.add(i);
        int tries = 0;
