...
final class ListenForInputDialog {
    ...
    static void show(KeyComboCallback resultCallback, Set<InputKey> displayCurrentKeyCombo) {
        ...
        for (InputKey key : displayCurrentKeyCombo) recordedKeys.add(key.code());
    }
    ...
    static void imgui() {
        ...
        if (!ImGui.isPopupOpen(PopupID)) closeDialog(false);
    }
    ...
    private static void recordInput() {
        ...
        for (int keyCode : Input.getModifierKeysCodes()) {
            if (KeyListener.isKeyPressed(keyCode)) newMods.add(keyCode);
        }
    ...
    private static void closeDialog(boolean confirmed) {
        ...
        InputMapTab.closeListenForInputDialog();
    }
}
