...
public class MouseListener {
    ...
    public static void endFrame() {
        ...
        if (LogicServer.currentScene() == null) return;
        listener.worldPastX = listener.worldCurrentX;
        listener.worldPastY = listener.worldCurrentY;
    }
    ...
    public static Vector2f getCursorWorldTraverse() {
        if (LogicServer.currentScene() == null) return new Vector2f();
        MouseListener listener = get();
        if (listener == null) return new Vector2f();
    ...
    public static boolean isButtonPressed(int keyCode) {
        MouseListener listener = get();
        if (listener == null || invalidKey(keyCode)) return false;
    ...
    public static Vector2f getScreenPosition() {
        Vector2f currentVP = get().currentViewportPosition;
        if (currentVP.x == 0.0f || currentVP.y == 0.0f) return new Vector2f(getX(), getY());
    ...
    public static Vector2f getWorldPosition() {
        ...
        if (camera == null) return new Vector2f();
    ...
    public static Vector2f screen2WorldCoordinate(Vector2f screenCoordinate) {
        if (LogicServer.currentScene() == null) return new Vector2f();
        Vector2f normalization = new Vector2f(screenCoordinate.x / Window.getWidth(), screenCoordinate.y / Window.getHeight());
        normalization.mul(2f).sub(1.0f, 1.0f);
        Viewport viewport = LogicServer.currentScene().viewport();
        if (viewport == null) return new Vector2f();
    ...
    public static Vector2f world2ScreenCoordinate(Vector2f worldCoordinate) {
        ...
        Vector2f windowSpace = new Vector2f(normalization.x, normalization.y).mul(1f / normalization.w);
        windowSpace.add(1.0f, 1.0f).mul(0.5f);
        windowSpace.mul(Window.getWidth(), Window.getHeight());
        return windowSpace;
    }

    private static boolean invalidKey(int keyCode) {
